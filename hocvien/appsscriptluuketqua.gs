...
function getOrCreateSheet_() {
  ...
  // Tab đã có sẵn từ trước (có thể tạo bởi bản code cũ, tên cột cũ) — TỰ KIỂM TRA
  // hàng 1 mỗi lần chạy, nếu khác với HEADERS hiện tại thì tự sửa lại, không đụng
  // tới dữ liệu các hàng khác. Nhờ vậy sau này đổi tên/thứ tự cột trong HEADERS ở
  // trên, không cần vào Sheet sửa tay hàng 1 nữa — lần nộp bài kế tiếp tự cập nhật.
  const currentHeaders = sheet.getRange(1, 1, 1, HEADERS.length).getValues()[0];
  const isSame = HEADERS.every(function (h, i) { return currentHeaders[i] === h; });
  if (!isSame) {
